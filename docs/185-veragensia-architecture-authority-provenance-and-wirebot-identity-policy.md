# 185 — Veragensia Architecture Authority, Provenance, and Wirebot Identity Policy

**Status:** LIVE CONSTITUTIONAL AUTHORITY CONTRACT

This policy governs architecture, product direction, trust boundaries, canonical specifications, authority semantics, and cross-system design decisions in Veragensia and its Focusa/UIAI Engine integrations.
...
## 3. Veragensia operational authority is not architecture authority

Veragensia owns the operating-environment substrate it is explicitly assigned: platform integration, service lifecycle, process/container boundaries, desktop/shell projection, hardware/node lifecycle, execution-environment health, and the safe presentation of Focusa/UIAI state.

That does not make Veragensia the owner of organizational canon, Focusa's reducer state, UIAI's browser runtime, or the estate's constitutional architecture.

```text
Veragensia node/runtime state    != canonical architecture authority
machine/root/process capability  != architecture authorization
Focusa operational truth         != Veragensia-owned organizational canon
UIAI browser truth               != Veragensia-owned browser implementation
issue/PR/commit provenance       != architecture authority
```

Veragensia MUST project and enforce owner-approved boundaries rather than invent a second authority system.
...
The existing lowercase `wirebot` Linux/service account used by Veragensia infrastructure is execution identity only and has **zero architecture authority**.
...
A material authority-policy change produces a new constitution hash and requires compatibility/redelegation handling.
...
Veragensia may own the node/OS execution substrate without owning organizational architecture.
