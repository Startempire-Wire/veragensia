...
Each fixed probe has a two-second deadline and a combined 256 KiB stdout/stderr limit; command/binding projections cap at 512 entries. These bounds accommodate the measured upstream `f4378f0` registry (283 records, 137,295 bytes), while larger output remains explicitly incomplete. Missing tools/session, unknown schemas, invalid output, truncation and unresolved defaults remain explicit. Unknown command-registry shapes are **not** guessed: the parser accepts a list of names/records or an object with a `commands` list. For upstream records it preserves the full `route` (falling back to `name` only when no route field exists). The JSON probe is `omarchy commands --all --json`: do not combine it with `--check`, which takes precedence and emits validation text instead of JSON in upstream revision `f4378f0`. A symlinked or non-regular override file is not followed. `--require-complete` exits 2 on incomplete observations; ordinary diagnostic mode still emits a degraded report and exits 0.
...
The `platform` section observes only the current host's OS, kernel release and architecture through the standard library; hostname and kernel build metadata are omitted. `manifest.architecture_match` is true/false for a known comparison, or null when either architecture is unknown. Mismatch adds a diagnostic gap. These are host observations, not native-target qualification; unavailable observations remain explicit and `release_ready` stays false.

`dependencies.focusa_cli` observes the installed CLI using the fixed, bounded `focusa --version` probe. `manifest.focusa_cli_version_match` compares its exact version string against the existing candidate pin; missing pins or unavailable/malformed output remain unknown. A mismatch adds a diagnostic gap. CLI version observation says nothing about daemon health, authentication, artifact integrity or native compatibility.
